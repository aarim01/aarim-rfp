...
`;
  ...
  async runMatchForUser(userId: string): Promise<{ success: boolean; message: string; companyId?: string }> {
    ...
    if (!company) {
      return {
        success: false,
        message: 'No company profile found. Please set up your company profile first to enable AI matching.',
      };
    }

    const tenders = await this.tenderRepository.find({ where: { status: 'open' } });
    this.logger.log(`Running AI match for company ${company.company_name} against ${tenders.length} open tenders`);

    for (const tender of tenders) {
      try {
        await this.calculateMatch(company.id, tender.id);
      } catch (error) {
        this.logger.error(`Error matching tender ${tender.id}: ${error.message}`);
      }
    }

    return {
      success: true,
      message: `AI matching complete. Evaluated ${tenders.length} open tenders.`,
