...
export class ScrapingService {
  ...
  async scrapeSource(sourceId: string): Promise<ScrapingResult> {
    ...
    try {
      ...
      // A scrape is only successful if it actually returned tenders
      const scrapeSucceeded = result.tenders.length > 0;
      if (!scrapeSucceeded) {
        const errMsg = result.errors.length > 0 ? result.errors.join('; ') : 'Scraper returned 0 tenders';
        this.logger.warn(`Scrape of ${source.name} returned 0 tenders: ${errMsg}`);
        await this.updateSourceHealth(sourceId, false, 0, errMsg);
      } else {
        await this.updateSourceHealth(sourceId, true, result.tenders.length);
      }

      this.logger.log(
        `Scraped ${source.name}: ${processed.newTenders} new, ${processed.duplicates} duplicates, ${processed.errors} errors`
      );
