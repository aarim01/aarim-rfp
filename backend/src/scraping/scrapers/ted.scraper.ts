...
export class TedScraper extends BaseScraper {
  ...
  private async fetchFromApi(): Promise<ScrapingResult> {
    ...
    // Filter to notices published in the last 30 days so we always get current tenders
  ...
  private mapNotice(notice: TedNotice): TenderData | null {
    ...
    return {
      ...
      country,
      category: (notice['procedure-type'] || 'Public Procurement').slice(0, 100),
      deadline,
      published_date: publishedDate,
      tender_number: pubNumber,
      source_url: sourceUrl.slice(0, 500),
      procurement_type: (notice['procedure-type'] || 'Open Procedure').slice(0, 100),
