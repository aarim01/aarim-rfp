...
export class AssistantService {
  ...
  // HTTP Chat (works without company profile)
  async chat(userId: string, message: string): Promise<{ reply: string }> {
    ...
    let context = '';
    if (companyProfile) {
      context = await this.buildContext(userId, companyProfile, message);
    } else {
      const tenderCount = await this.tenderRepository.count();
      context = `No company profile configured yet. There are ${tenderCount} active tenders in the database. Encourage the user to set up their company profile to enable AI matching.`;
    }

    const reply = await this.getAIAnswer(message, context);

    await this.aiConversationRepository.save({
      user_id: userId,
      question: message,
      answer: reply,
      context: { http_chat: true },
    });
...
`;
      ...
      // Fallback: save a structured template proposal when AI is unavailable
      const fallbackDraft = `# Proposal Draft for: ${tender.title}

## 1. Executive Summary
${company.company_name} is pleased to submit this proposal in response to the tender issued by ${tender.organization || 'the issuing organization'}. As a ${company.industry} company specializing in ${company.services?.join(', ')}, we are well-positioned to deliver the required services.

## 2. Alignment with Tender Requirements
Our expertise in ${company.services?.join(', ')} directly aligns with the requirements outlined in this tender. ${company.description || ''}

## 3. Proposed Solution & Scope of Work
We propose a comprehensive solution leveraging our capabilities in ${company.services?.join(' and ')}. Our approach ensures quality delivery, on-time completion, and full compliance with all tender specifications.

## 4. Qualifications & Past Experience
${company.company_name} brings proven experience in ${company.industry}. Our team has the technical expertise and project management capabilities required to successfully execute this contract.

## 5. Pricing Methodology & Contact Details
Our pricing will be competitive and transparent. Please contact us at ${company.website_url || 'our offices'} to discuss detailed pricing.

---
*Note: Configure a valid OPENAI_API_KEY for a fully AI-generated proposal draft.*`;

      const proposal = this.tenderProposalRepository.create({
        company_id: companyId,
        tender_id: tenderId,
        proposal_draft: fallbackDraft,
      });
      return this.tenderProposalRepository.save(proposal);
    }
  }
